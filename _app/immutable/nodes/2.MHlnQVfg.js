...
`)[0];for(let d=0;d<p.length;d++){const k=p.charAt(d);if(!y(s).includes(k)&&!m.includes(k))return!1}return!0})),x=sa(()=>r.realWords&&y(f).length>0||!r.realWords&&y(s).length>0);function N(){aa(t,!0),J()}function I(){aa(t,!1)}function w(h){const m=k=>Ga.find(v=>v.character===k)?.reading,p=["ゃ","ゅ","ょ"];let d="";for(let k=0;k<h.length;k++){const v=h.charAt(k),_=h.charAt(k+1),M=m(v);p.includes(v)||v==="ー"||(v==="っ"&&_?d+=m(_)?.charAt(0):_&&_==="ー"?(d+=M,d+=M?.charAt(M?.length-1)):_&&p.includes(_)?d+=m(v+_):M&&(d+=M))}return d}function J(){let h="",m="",p="";if(r.realWords){const d=y(f)[Math.floor(Math.random()*y(f).length)];h=d.hiragana,m=w(d.hiragana),d.examples&&(p+=`Meaning: "${d.examples.split(`
`)[0]}"`)}else{let d=()=>y(g)[Math.floor(Math.random()*y(g).length)];for(let k=1;k<=r.wordLength;k++){const v=d();h+=v.character,m+=v.reading}}r.promptRomaji?aa(i,{word:m,alt:h,extra:p},!0):aa(i,{word:h,alt:m,extra:p},!0),aa(o,!1)}function S(){aa(o,!0)}var l=Oa();Ee("1uha8ag",h=>{Pe(()=>{Ce.title="Japanese Kana Drill"})});var b=Za(l);{var B=h=>{var m=on(),p=c(m),d=c(p),k=c(d,!0);u(d);var v=q(d,2);let _;var M=c(v,!0);u(v);var D=q(v,2);{var F=C=>{var E=tn();let Y;var V=c(E,!0);u(E),Q(()=>{Y=na(E,1,"extra svelte-1uha8ag",null,Y,{hidden:!y(o)}),O(V,y(i).extra)}),T(C,E)};X(D,C=>{y(i).extra&&C(F)})}u(p);var j=q(p,2),A=c(j);A.__click=J;var P=q(A,2);let R;P.__click=S;var G=q(P,2);G.__click=I,u(j),u(m),Q(()=>{O(k,y(i).word),_=na(v,1,"word-alt svelte-1uha8ag",null,_,{hidden:!y(o)}),O(M,y(i).alt),R=na(P,1,"reveal svelte-1uha8ag",null,R,{revealed:y(o)})}),T(h,m)},z=h=>{var m=un(),p=q(c(m),2),d=c(p),k=c(d);nn(k,{get showKatakana(){return n.katakana},get selectedGojuuon(){return n.selectedGojuuon},set selectedGojuuon(W){n.selectedGojuuon=W}}),u(d);var v=q(d,2),_=c(v),M=c(_);oa(M,{get checked(){return n.katakana},set checked(W){n.katakana=W}}),Z(2),u(_);var D=q(_,4),F=c(D);oa(F,{get checked(){return n.dakuten},set checked(W){n.dakuten=W}}),Z(2),u(D);var j=q(D,2),A=c(j);oa(A,{get checked(){return n.yoon},set checked(W){n.yoon=W}}),Z(2),u(j);var P=q(j,4);let R;var G=c(P),C=q(c(G)),E=c(C,!0);u(C),Z(3),u(G);var Y=q(G,2);Qe(Y,{get disabled(){return r.realWords},min:1,max:10,get value(){return r.wordLength},set value(W){r.wordLength=W}}),u(P);var V=q(P,2),la=c(V);oa(la,{get checked(){return r.realWords},set checked(W){r.realWords=W}});var ra=q(la,2),ua=q(c(ra),3),ka=q(c(ua),2);u(ua),u(ra),u(V);var Pa=q(V,2),ee=c(Pa);oa(ee,{get checked(){return r.promptRomaji},set checked(W){r.promptRomaji=W}}),Z(2),u(Pa),u(v);var Ca=q(v,2),xa=q(c(Ca),2),Ea=c(xa),ne=c(Ea,!0);u(Ea),Z(),u(xa);var Na=q(xa,2);fa(Na,21,()=>y(s),ma,(W,ia)=>{var ta=sn(),ja=c(ta,!0);u(ta),Q(()=>O(ja,y(ia))),T(W,ta)}),u(Na),u(Ca),u(p);var va=q(p,2);va.__click=N;var re=q(va,2);{var ie=W=>{var ia=ln(),ta=c(ia);{var ja=$=>{var wa=ga("Please select characters to start.");T($,wa)},te=$=>{var wa=hn();Z(2),T($,wa)};X(ta,$=>{y(s).length==0?$(ja):$(te,!1)})}u(ia),T(W,ia)};X(re,W=>{y(x)||W(ie)})}u(m),Q(()=>{R=na(P,1,"word-length svelte-1uha8ag",null,R,{disabled:r.realWords}),O(E,r.wordLength),O(ka,` (${y(f).length??""} words matched).`),O(ne,y(s).length),va.disabled=!y(x)}),T(h,m)};X(b,h=>{y(t)?h(B):h(z,!1)})}T(a,l),pa()}$a(["click"]);export{yn as component};
